    ...
    public String payNotify(HttpServletRequest request) throws Exception {
        if (request.getParameter("trade_status").equals("TRADE_SUCCESS")) {
            ...
            if (checkSignature) {
                ...
                // 查询订单
                QueryWrapper<Df> queryWrapper = new QueryWrapper<>();
                queryWrapper.eq("order_id", outTradeNo);
                Df df= dfMapper.selectOne(queryWrapper);
                System.out.println(df);
                if (df != null) {
                    System.out.println(df);
                    df.setAlipayNo(alipayTradeNo);
                    df.setPayTime(new Date());
                    df.setPaymentStatus("已支付");
                    System.out.println(df);
